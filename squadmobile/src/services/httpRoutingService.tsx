...
class httpRoutingService {
    ...
    getMethod(url: string, queryParams?: any) {
        if (needConsole) console.log("Inside get method", url, queryParams)
    ...
    postMethod(url: string, data: any, queryParams?: any) {
        if (needConsole) console.log("inside post data", url, data, queryParams);
    ...
    deleteMethod(url: string) {
        if (needConsole) console.log("inside delete data", url);
        return axiosInstance.delete(config.apiUrl + url)
    }
    putMethod(url: string, data: any) {
        if (needConsole) console.log("inside put data", url, data);
